...
function updateScore(isOut = false) {
  ...
  if (isOut) {
    ...
    if (outReason == "catchOut" || outReason == "bowled") {
      ...
      console.log(outPlayerIndex);
      console.log(allPlayerDetails[outPlayerIndex]);
      console.log(allPlayerDetails[outPlayerIndex].balls);
  ...
  for (let j = 0; j < allPlayerDetails.length; j++) {
    ...
    const avg = isNaN(allPlayerDetails[j].runs / allPlayerDetails[j].balls)
      ? "0.00"
      : (allPlayerDetails[j].runs / allPlayerDetails[j].balls).toFixed(2);
  ...
  if (
    (ball == "noBall") && outReason == "catchOut" ||
    outReason == "bowled"
  ) {
    newRun = "";
    firstNoBall = "N.Out";
