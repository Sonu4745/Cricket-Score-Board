...
function updateScore(isOut = false) {
    ...
    let nonStrikePlayer = allPlayerDetails.find(el => (el.playingStatus == 1 && el.onstrike == false));
  ...
  <th> Team Avrg</th>
    ...
    if (isOut) {
        // 0 runs for case of catch out and bowled
    ...
    for (let j = 0; j < allPlayerDetails.length; j++) {
        ...
        let onStrikeStatus = allPlayerDetails[j].playingStatus == 1 && allPlayerDetails[j].onstrike == true ? "On Strike" :
            (allPlayerDetails[j].playingStatus == 1 && allPlayerDetails[j].onstrike == false ? "Non Strike" : "");
        let bgColorOut;
        let txtColor;
        ...
        if (allPlayerDetails[j].playingStatus == 1 && allPlayerDetails[j].onstrike == true) {
            bgColorOut = '#55945f';
            txtColor = 'white'
        }
        if (allPlayerDetails[j].playingStatus == 1 && allPlayerDetails[j].onstrike == false) {
            bgColorOut = '#99a899';
            txtColor = 'white'
        }
        const avg = isNaN((allPlayerDetails[j].runs / allPlayerDetails[j].balls)) ? "0.00" : (allPlayerDetails[j].runs / allPlayerDetails[j].balls).toFixed(2);
    ...
    if (wicket && isOut) {
        if (ball == "noBall" && outReason == "catchOut" || outReason == "bowled") {
            newRun = '';
            firstNoBall = 'not.Out';
            wicket--;
            teamSummaryObject.wicket--;
        }
        else if (outReason == "catchOut") {
            newRun = '';
            firstNoBall = 'C.Out';
    ...
    let teamAvrg = teamSummaryObject.runs / totalTeamBalls;
